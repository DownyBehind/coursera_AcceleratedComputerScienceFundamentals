...
std::vector<T> traverseLevels(GenericTree<T> &tree)
{
  ...
  std::queue<TreeNode *> level_node; // queue
  int queue_size;
  int vector_size;
  TreeNode *temp_node;

  level_node.push(tree.getRootPtr());
  results.push_back(tree.getRootPtr()->data);

  while (!level_node.empty())
  {
    queue_size = level_node.size();
    for (int i = 0; i < queue_size; ++i)
    {
      temp_node = level_node.front();

      level_node.pop();

      vector_size = temp_node->childrenPtrs.size();

      for (int j = 0; j < vector_size; ++j)
      {

        results.push_back(temp_node->childrenPtrs[j]->data);
        level_node.push(temp_node->childrenPtrs[j]);
      }
    }
  }
  // ...
